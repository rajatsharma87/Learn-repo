    ...
    def login(self):
        ...
        match login_option:
            case 1:  # This section is for authorised user to login into the application
                ...
                user_df = pd.read_csv('D:\G\Barn\\auth\\user_list.csv')
                # for row in reader:
                ...
                if not nf_user_df.empty:
                    check_user_type = nf_user_df['type'] # this gets what type of user is this admin/customer
                    print(check_user_type)
                    if nf_user_df.loc[(nf_user_df['type'] == 'admin')]:  # if admin user then the admin func is called from the product class
                        y=admin()
                        y.admin_task()
                    elif nf_user_df.loc[(nf_user_df['type'] == 'cust')]: # if Customer user then product func is called from prodcut class
                        print("##########################################\n\n")
                        print("\t" + auth.wel_note)
